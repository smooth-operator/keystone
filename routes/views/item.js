...
exports = module.exports = function(req, res) {
	console.log(req.list.path);
	
	var User = keystone.list('User');
	User.model.findById(req.user.id).select().exec(function(err, user){
		if (user.canAccessKeystone || (!user.canAccessKeystone && (req.list.path=='meetups' || req.list.path=='talks' || req.list.path=='rsvps' || req.list.path=='groups' || req.list.path=='posts')) ){
